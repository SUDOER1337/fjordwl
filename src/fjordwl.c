...
static void applybounds(Client *c,
						struct wlr_box *bbox); // ,
static void applyrules(Client *c);			   // ,config.h
...
struct fjordwl_print_status_manager *print_status_manager;
...
void applyrules(Client *c) {
	...
	// if no geom rule hit and is normal window, use the center pos and record
	// the hit size
	if (!c->iscustompos &&
...
void maplayersurfacenotify(struct wl_listener *listener, void *data) {
	...
	strncpy(l->mon->last_surface_ws_name, layer_surface->namespace,
			sizeof(l->mon->last_surface_ws_name) - 1); // 255
...
void createswitch(struct wlr_switch *switch_device) {
	...
	if (wlr_input_device_is_libinput(&switch_device->base) &&
		...
		input_dev->libinput_device = device;
		input_dev->device_data = NULL; //  NULL
...
mapnotify(struct wl_listener *listener, void *data) {
	...
	if (config.new_is_master && selmon && !is_scroller_layout(selmon))
		// tile at the top
		wl_list_insert(&clients, &c->link); // master,
...
// printstatus，
void printstatus(void) { wl_signal_emit(&fjordwl_print_status, NULL); }
...
void setup(void) {
	...
	//  setup
	wl_signal_init(&fjordwl_print_status);
	wl_signal_add(&fjordwl_print_status, &print_status_listener);
...
void overview_restore(Client *c, const Arg *arg) {
	...
	if (c->bw == 0 && !c->isfullscreen) { // ov,bw
		c->bw = c->isnoborder ? 0 : config.borderpx;
	}
